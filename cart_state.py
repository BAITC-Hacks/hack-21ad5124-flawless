...
class CartStateCodec:
    ...
    def encode(self, tools: ShopTools, session_id: str) -> str:
        with tools.lock:
            ...
            processed = sorted([article, turn] for sid, article, turn in tools.processed_additions if sid == session_id)
        payload = {"v": 1, "sid": session_id, "cart": cart, "processed": processed, "iat": int(datetime.now(timezone.utc).timestamp())}
    ...
    def decode(self, token: str, session_id: str | None = None) -> dict:
        ...
        if (payload.get("v") != 1 or not isinstance(payload.get("sid"), str) or not payload["sid"]
    ...
    def restore(self, tools: ShopTools, token: str, session_id: str | None = None) -> str:
        ...
        for item in payload["processed"]:
            if (not isinstance(item, list) or len(item) != 2 or not isinstance(item[0], str)
                    or not isinstance(item[1], str) or len(item[1]) != 64):
                raise InvalidCartToken("Invalid cart history")
            processed.add((session_id, item[0], item[1]))
        with tools.lock:
