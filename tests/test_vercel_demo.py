    ...
    def test_live_catalog_still_uses_assistant_cart_and_hides_ekt_link(self):
        def load_sample(catalog):
            catalog._set_products([normalize_product({
                "id": 1, "article": "EKT-TEST", "name": "Тестовый выключатель", "category": "Низковольтная аппаратура",
        ...
        with patch.dict(os.environ, settings), patch("main.Catalog.load", new=load_sample), patch("main.run_openai_agent", side_effect=model_reply):
            with TestClient(app) as client:
                ...
                self.assertIn("Тестовый выключатель", client.get(data["cart_link"]).text)
                self.assertNotIn("EKT-TEST", client.get(data["cart_link"]).text)
    ...
    def test_cart_survives_new_function_instance_and_link_shows_items(self):
        ...
        with patch.dict(os.environ, settings):
            with TestClient(app) as client:
                ...
                self.assertIn("Автоматический выключатель 1P 16 А", client.get(data["cart_link"]).text)
                self.assertNotIn("DEMO-AV-16", client.get(data["cart_link"]).text)
